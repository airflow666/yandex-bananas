  ...
  async _load(status) {
    ...
    res.entries.slice(0, 10).forEach((e, i) => {
      ...
      bg.fillStyle(isMe ? 0xffd93b : 0x081c15, isMe ? 0.25 : 0.7)
        .fillRoundedRect(24, y, GAME_W - 48, 52, 12);

      const medals = ['🥇', '🥈', '🥉'];
      const rankLabel = e.rank <= 3 ? medals[e.rank - 1] : `${e.rank}.`;
      this.add.text(46, y + 26, rankLabel, {
        fontFamily: FONT, fontSize: '22px', color: '#ffe680', fontStyle: 'bold',
      }).setOrigin(0, 0.5);

      const name = isMe ? `${e.name} (${t('yourResult')})` : e.name;
      this.add.text(105, y + 26, name, {
        fontFamily: FONT, fontSize: '20px', color: isMe ? '#ffe680' : '#ffffff',
      }).setOrigin(0, 0.5);

      this.add.text(GAME_W - 46, y + 26, String(e.score), {
        fontFamily: FONT, fontSize: '22px', color: '#95d5b2', fontStyle: 'bold',
      }).setOrigin(1, 0.5);
    });
  }
}
